...
class ConvTransposeBlock(nn.Module):
    def __init__(self, in_channel, kernel_size, filters, upsample_stride=(2, 2, 2)):
        super(ConvTransposeBlock, self).__init__()
        F1, F2, F3 = filters
        if isinstance(upsample_stride, int):
            upsample_stride = (upsample_stride,) * 3
        self.stage = nn.Sequential(
            nn.ConvTranspose3d(
                in_channel,
                F1,
        ...
        self.shortcut_1 = nn.ConvTranspose3d(
            in_channel,
            F3,
            kernel_size=upsample_stride,
            stride=upsample_stride,
...
class OCBE(nn.Module):
    def __init__(self, in_channels_list=[480, 832, 1024]):
        ...
        self.branch2 = nn.Sequential(
            nn.Conv3d(in_channels_list[1], 256, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm3d(256),
            nn.ReLU(True),
        ...
        self.branch3 = nn.Sequential(
            nn.Conv3d(in_channels_list[2], 512, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(512),
            nn.ReLU(True),
        )
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        # Bottleneck -> feature3. Only the spatial dimensions are upsampled so
        # that the temporal resolution stays aligned with the I3D feature.
        ...
        self.match_feature3 = nn.Conv3d(1024, 1024, kernel_size=1)

        # feature3 -> feature2. Temporal and spatial dimensions are doubled.
        self.layer2 = nn.Sequential(
            ConvTransposeBlock(in_channel=1024, kernel_size=3, filters=[256, 512, 512]),
            IdentityBlock(in_channel=512, kernel_size=3, filters=[256, 512, 512]),
            IdentityBlock(in_channel=512, kernel_size=3, filters=[256, 512, 512]),
        )
        self.match_feature2 = nn.Conv3d(512, 832, kernel_size=1)

        # feature2 -> feature1. Temporal and spatial dimensions are doubled.
        self.layer1 = nn.Sequential(
            ConvTransposeBlock(in_channel=512, kernel_size=3, filters=[128, 256, 256]),
            IdentityBlock(in_channel=256, kernel_size=3, filters=[128, 256, 256]),
        )
        self.match_feature1 = nn.Conv3d(256, 480, kernel_size=1)
...
class OcbeAndDecoder(nn.Module):
    ...
    def forward(self, e_feature1, e_feature2, e_feature3):
        ocbe_output = self.ocbe(e_feature1, e_feature2, e_feature3)  # (B, 1024, 2, 4, 4)
