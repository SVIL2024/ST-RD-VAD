...
def _build_fg_mask(images, out_hw=64, q=0.85, min_area_ratio=0.01, bg_mix=0.7):
    """
...
def Test(
    ...
    with torch.no_grad():
        for idx, sample in enumerate(tqdm(test_loader, desc="Testing", leave=False, disable=True)):
            ...
            score_map = 0.5 * m1 + 1.0 * m2 + 1.2 * m3 + 1.0 * torch.log1p(e3)  # (T, 64, 64)

            if use_fg_mask:
                fg_mask = _build_fg_mask(images, out_hw=64, q=fg_q, min_area_ratio=min_area_ratio)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fg_q", type=float, default=0.87)
